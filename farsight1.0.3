...
def launch_game(account):
    ...
    time.sleep(7)  # Wait for Riot Client to load
...
def find_account_name(game_name):
    try:
        with open(DATA_FILE, 'r') as f:
            ...
            for account in accounts:
                ...
                if isinstance(account, dict):
                    if account.get('gameName') == game_name:
                        return account.get('account_name')
                # Check if account is a string (fallback)
        ...
        print("Error decoding accounts.json")
    return None

def find_lockfile(_=None):
    global lockfile_path, lockfile_found
    if lockfile_found:
        print("Lockfile already found, skipping search.")
        return
                ...
                if "LeagueClient" in proc.info["name"]:
                    if proc.info["cwd"]:
                        ...
                        if os.path.exists(lockfile_path):
                            print(f"Lockfile path found: {lockfile_path}")
                            lockfile_found = True  # Set the flag to prevent re-entry
                            get_summoner_info()  # Fetch gameName once lockfile is found
...
def get_summoner_info():
    ...
    try:
        if not lockfile_path or not os.path.exists(lockfile_path):
            print("Lockfile not found or invalid path.")
            return None

        with open(lockfile_path, "r") as lockfile:
            content = lockfile.read().strip().split(":")
            if len(content) < 5:
                print(f"Invalid lockfile format: {content}")
                return None
            _, _, port, auth_token, protocol = content
        ...
        headers = {
            ...
            # Retry logic to handle "You are not logged in" error
        for attempt in range(10):
            try:
                ...
                if response.status_code == 200:
                    ...
                    game_name = summoner_info.get('gameName', 'UnknowngameName')

                    print(f"Fetched summoner info: {summoner_info['gameName']} (Level {summoner_info['summonerLevel']})")
                    update_accounts_json_with_gameName(account_name, game_name)
                    return summoner_info

                elif response.status_code == 404 and "You are not logged in." in response.text:
                    print(f"Attempt {attempt + 1}: League Client is not logged in. Retrying in 3 seconds...")
                    time.sleep(3)
        ...
        print("Max retries reached. Unable to fetch summoner info.")
        return None

    except Exception as e:
        print(f"Exception in get_summoner_info: {e}")
        return None

def update_accounts_json_with_gameName(account_name, game_name):
    accounts = load_accounts()
    updated = False

    for account in accounts:
                if account["account_name"] == account_name:
                    account["gameName"] == game_name
                    updated = True
                    break
            
    if not updated:
                accounts.append({"account_name": account_name, "gameName": game_name})
...
def update_account_rank(account_name, new_rank):
    ...
    launch_client_menu(lockfile_path)
...
def launch_client_menu(lockfile_path):
    ...
    winrate = round((win_count / total_matches) * 100, 2) if total_matches > 0 else 0.00
...
def initialize_app(accounts):
    ...
    lockfile_thread = threading.Thread(target=find_lockfile, daemon=True)
    lockfile_thread.start()
